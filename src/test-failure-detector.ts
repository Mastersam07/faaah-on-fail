import * as vscode from 'vscode';

const BUILT_IN_PATTERNS = [
  /\btest\b/i,
  /\bspec\b/i,
  /\bcheck\b/i,
  /\bpytest\b/i,
  /\bjest\b/i,
  /\bvitest\b/i,
  /\bmocha\b/i,
  /\brspec\b/i,
  /\bcargo\s+test\b/i,
  /\bgo\s+test\b/i,
  /\bflutter\s+test\b/i,
  /\bdart\s+test\b/i,
  /\bphpunit\b/i,
  /\bgradle\s+test\b/i,
  /\bmvn\s+test\b/i,
  /\bdotnet\s+test\b/i,
  /\bnx\s+test\b/i,
  /\bexunit\b/i,
  /\bbun\s+test\b/i,
  /\bdeno\s+test\b/i,
  /\bcypress\s+run\b/i,
  /\bjasmine\b/i,
  /\bkarma\b/i,
  /\bava\b/i,
  /\btox\b/i,
  /\bnox\b/i,
  /\bctest\b/i,
  /\bmaestro\b/i,
];

const DEBOUNCE_MS = 2000;

export class TestFailureDetector {
  private disposables: vscode.Disposable[] = [];
  private lastTriggerTime = 0;
  private onFailure: () => void;

  constructor(onFailure: () => void) {
    this.onFailure = onFailure;
  }

  activate(): void {
    if (vscode.window.onDidEndTerminalShellExecution) {
      this.disposables.push(
        vscode.window.onDidEndTerminalShellExecution((e) => {
          if (e.exitCode !== undefined && e.exitCode !== 0) {
            const cmd = e.execution.commandLine?.value ?? '';
            if (this.isTestCommand(cmd)) {
              this.trigger();
            }
          }
        })
      );
    }

    this.disposables.push(
      vscode.tasks.onDidEndTaskProcess((e) => {
        if (e.exitCode !== undefined && e.exitCode !== 0) {
          const task = e.execution.task;
          if (task.group === vscode.TaskGroup.Test || this.isTestTask(task)) {
            this.trigger();
          }
        }
      })
    );

    // TODO(mastersam): Find alternative on working with test explorer as `onDidChangeTestResults` on the test API is proposal based.
  }

  private getExtraPatterns(): RegExp[] {
    const extras = vscode.workspace
      .getConfiguration('faaaahOnFail')
      .get<string[]>('extraTestCommands', []);
    return extras.map((ec) => {
      const escaped = ec.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      return new RegExp(`\\b${escaped}\\b`, 'i');
    });
  }

  private isTestCommand(cmd: string): boolean {
    if (BUILT_IN_PATTERNS.some((p) => p.test(cmd))) {
      return true;
    }
    return this.getExtraPatterns().some((p) => p.test(cmd));
  }

  private isTestTask(task: vscode.Task): boolean {
    const name = task.name;
    if (this.isTestCommand(name)) { return true; }

    const execution = task.execution;
    if (execution instanceof vscode.ShellExecution) {
      return this.isTestCommand(this.extractCommand(execution));
    }
    if (execution instanceof vscode.ProcessExecution) {
      return this.isTestCommand(`${execution.process} ${execution.args.join(' ')}`);
    }

    return false;
  }

  private extractCommand(execution: vscode.ShellExecution): string {
    if (typeof execution.commandLine === 'string') {
      return execution.commandLine;
    }
    if (execution.command) {
      const cmd = typeof execution.command === 'string'
        ? execution.command
        : execution.command.value;
      const args = (execution.args || [])
        .map((a: string | vscode.ShellQuotedString) =>
          typeof a === 'string' ? a : a.value
        )
        .join(' ');
      return `${cmd} ${args}`;
    }
    return '';
  }

  private trigger(): void {
    const now = Date.now();
    if (now - this.lastTriggerTime < DEBOUNCE_MS) {
      return;
    }
    this.lastTriggerTime = now;
    this.onFailure();
  }

  dispose(): void {
    this.disposables.forEach((d) => d.dispose());
    this.disposables = [];
  }
}
